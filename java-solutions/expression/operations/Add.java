...
public class Add<T extends Number> extends BinaryOperation<T> implements MathExpression<T> {

    public Add(MathExpression<T> lhs, MathExpression<T> rhs) {
        super(lhs, rhs);
    }
    ...
    public T evaluate(Num<T> cl, T x, T y, T z) throws EvaluateException {
        return cl.add(getLhs().evaluate(cl, x, y, z), getRhs().evaluate(cl, x, y, z));
    }
