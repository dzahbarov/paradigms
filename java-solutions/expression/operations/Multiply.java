...
public class Multiply<T extends Number> extends BinaryOperation<T> implements MathExpression<T> {

    public Multiply(MathExpression<T> lhs, MathExpression<T> rhs) {
        super(lhs, rhs);
    }
    ...
    public T evaluate(Num<T> cl, T x, T y, T z) throws EvaluateException {
        return cl.multiply(getLhs().evaluate(cl, x, y, z), getRhs().evaluate(cl, x, y, z));
    }
